...
export class StreamingIngestionConstruct extends Construct {
  ...
  constructor(parent: Construct, name: string, props: StreamingIngestionConstructProps) {
    super(parent, name);
    ...
    const firehouseS3DeliveryLogStream = new logs.LogStream(this, 'firehose-s3-delivery-log-stream', {
      logGroup: firehoseLogGroup,
    });

    const firehouseBackupDeliveryLogStream = new logs.LogStream(this, 'firehose-backup-delivery-log-stream', {
      logGroup: firehoseLogGroup,
    });
    ...
    // Role for firehose
    const gamesEventsFirehoseRole = new iam.Role(this, 'games-events-firehose-role', {
        ...
        firehose_delivery_policy: new iam.PolicyDocument({
          statements: [
            new iam.PolicyStatement({
              ...
              resources: [props.analyticsBucket.arnForObjects('*'), props.analyticsBucket.bucketArn],
            }),
            new iam.PolicyStatement({
    ...
    const s3TimestampPrefix = 'year=!{timestamp:YYYY}/month=!{timestamp:MM}/day=!{timestamp:dd}';
    ...
    if (
      ...
      props.gamesEventsStream instanceof cdk.aws_kinesis.Stream
    ) {
    ...
    var firehoseDestinationConfiguration = props.config.ENABLE_APACHE_ICEBERG_SUPPORT
      ...
      : {
          extendedS3DestinationConfiguration: {
            ...
            prefix: `${props.config.RAW_EVENTS_PREFIX.replace(/\/+$/, '')}/year=!{partitionKeyFromQuery:year}/month=!{partitionKeyFromQuery:month}/day=!{partitionKeyFromQuery:day}/`,
    ...
    const gameEventsFirehose = new kinesisFirehose.CfnDeliveryStream(this, 'game-events-firehose', firehoseSettings);
