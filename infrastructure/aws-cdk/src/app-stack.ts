...
export class InfrastructureStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: InfrastructureStackProps) {
    ...
    var dataLakeConstruct;
    if (props.config.DATA_STACK === "DATA_LAKE") {
      // Glue datalake and processing jobs
      dataLakeConstruct = new DataLakeConstruct(this, "DataLakeConstruct", {
    ...
    if (props.config.ENABLE_QUICKSIGHT_DASHBOARD) {
      const quicksightConstruct = new QuickSightConstruct(this, "QuickSightConstruct", {
        config: props.config,
        redshiftConstruct: redshiftConstruct,
        vpcConstruct: vpcConstruct,
        dataLakeConstruct: props.config.DATA_STACK === "DATA_LAKE" ? dataLakeConstruct : undefined,
        analyticsBucket: props.config.DATA_STACK === "DATA_LAKE" ? analyticsBucket : undefined,
      });
      ...
      // The role name is generated by CDK — extract it from the construct
      lambdaConstruct.applicationAdminServiceFunction.addEnvironment(
        "QUICKSIGHT_ROLE_NAME", quicksightConstruct.qsRoleName
      );

      // Grant the admin Lambda permissions to delete the VPC connection and clean up the IAM role
      lambdaConstruct.applicationAdminServiceFunction.addToRolePolicy(
        new iam.PolicyStatement({
          sid: "QuickSightTeardown",
          effect: iam.Effect.ALLOW,
          actions: [
          ...
          resources: [
            `arn:aws:quicksight:${cdk.Aws.REGION}:${cdk.Aws.ACCOUNT_ID}:vpcConnection/${vpcConnectionId}`,
          ],
        })
      );
      lambdaConstruct.applicationAdminServiceFunction.addToRolePolicy(
        new iam.PolicyStatement({
          sid: "QuickSightTeardownIAM",
          effect: iam.Effect.ALLOW,
          actions: [
            "iam:DeleteRole",
