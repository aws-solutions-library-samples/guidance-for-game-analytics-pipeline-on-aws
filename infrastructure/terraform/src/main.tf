...
module "redshift_construct" {
  ...
  stack_name = local.config.WORKLOAD_NAME
  vpc_id = module.vpc_construct.vpc_id
  game_events_stream_arn = aws_kinesis_stream.game_events_stream[0].arn
...
module "data_processing_construct" {
  ...
  stack_name = local.config.WORKLOAD_NAME
  events_database = module.data_lake_construct.game_events_database
  raw_events_table_name = local.config.RAW_EVENTS_TABLE
...
module "athena_construct" {
  ...
  game_events_workgroup = module.data_lake_construct[0].game_analytics_workgroup.id
  raw_events_table = module.data_lake_construct[0].aws_glue_catalog_table.raw_events_table.name
}
...
module "dashboard_construct" {
  ...
  redshift_namespace_db_name          = module.redshift_construct[0].awscc_redshiftserverless_namespace.name
  redshift_workgroup_name             = module.redshift_construct[0].aws_redshiftserverless_workgroup.name
  data_platform_mode                  = local.config.DATA_PLATFORM_MODE
  real_time_analytics                 = local.config.REAL_TIME_ANALYTICS
}