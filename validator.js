...
assert(config.INGEST_MODE == "KINESIS_DATA_STREAMS" || config.INGEST_MODE == "DIRECT_BATCH" || config.INGEST_MODE == "KAFKA", "INGEST_MODE needs to be set to KINESIS_DATA_STREAMS, KAFKA, or DIRECT_BATCH")
// ingest mode needs to be set to kinesis data streams for real time analytics to work
assert(!config.REAL_TIME_ANALYTICS || ((config.INGEST_MODE == "KINESIS_DATA_STREAMS" && config.INGEST_MODE == "KAFKA") && config.REAL_TIME_ANALYTICS), "REAL_TIME_ANALYTICS can only be enabled if INGEST_MODE is set to KINESIS_DATA_STREAMS or KAFKA")
// kinesis needs to be enabled for redshift to work
assert(config.DATA_STACK == "DATA_LAKE" || config.DATA_STACK == "REDSHIFT", "DATA_STACK can only be set to REDSHIFT or DATA_LAKE")
assert(config.DATA_STACK == "DATA_LAKE" || (config.INGEST_MODE == "KINESIS_DATA_STREAMS" && config.DATA_STACK == "REDSHIFT"), "DATA_STACK can only be set to REDSHIFT if INGEST_MODE is set to KINESIS_DATA_STREAMS")
// iceberg will only be enabled if data lake is enabled
assert(!config.ENABLE_APACHE_ICEBERG_SUPPORT || (config.ENABLE_APACHE_ICEBERG_SUPPORT && config.DATA_STACK == "DATA_LAKE"), "ENABLE_APACHE_ICEBERG_SUPPORT cannot be enabled if the DATA_STACK is set to REDSHIFT")
assert(!config.ENABLE_S3_TABLES || (config.ENABLE_S3_TABLES && config.DATA_STACK == "DATA_LAKE"), "ENABLE_S3_TABLES cannot be enabled if the DATA_STACK is set to REDSHIFT")
